...
public class Scale1_ws : MonoBehaviour {
	...
	void Start () {
		...
		void Update () {
		selectObj = script.GetComponent<WorkspaceControl> ().getSelected ();
		wsCopy = script.GetComponent<WorkspaceControl> ().getCopy ();
			...
			if (cur_pos!=new Vector3(1000f, 1000f, 1000f)&&selectObj != null) {
				if (prev_pos != new Vector3 (1000f, 1000f, 1000f)) {
					...
					if (scale > 1) {
					wsCopy.transform.localScale = new Vector3 (scale/5f, scale/5f, scale/5f);
						prev_pos = cur_pos;
					} else if (scale < -1) {
					wsCopy.transform.localScale = new Vector3 (0.1f / -scale, 0.1f / -scale, 0.1f / -scale);
						prev_pos = cur_pos;
					} else {
					}
				} else {
			...
			selectObj.transform.localScale = wsCopy.transform.localScale;
			}
		}
